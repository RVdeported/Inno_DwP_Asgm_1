...
class decorator_3:
    ...
    @staticmethod
    def display_ranks():
        display_ranks(decorator_3)


def display_ranks(decorator):
    decorator.ranks = dict(sorted(zip(list(decorator.ranks), decorator.ranks.values()), key=lambda item: item[1]))
    name = decorator.ranks.keys()
    rank = [i for i in range(1, len(decorator.ranks)+1)]
    time = decorator.ranks.values()
    print(tabulate(zip(name, rank, time),
                   headers=['PROGRAM', 'RANK', 'TIME ELAPSED'], floatfmt=".6f", numalign="right"))
